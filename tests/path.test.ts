...
import { appPath, basePath, fileName, folderName, publicPath } from "@/lib/support/path";
...
it("will return base path (root of the project)", () => {
  ...
  expect(basePath("/benchmark/index.ts")).toBe(basePath(path.join("benchmark", "index.ts")));
});

it("will return app path", () => {
  expect(appPath()).toContain("app");
  expect(appPath("dashboard")).toBe(basePath(path.join("app", "dashboard")));
});

it("will return public path", () => {
  expect(publicPath()).toContain("public");
  expect(publicPath("image")).toBe(basePath(path.join("public", "image")));
});
...
it("will access the directory name from a path", () => {
  expect(folderName("src/support/array/")).toBe(basePath(path.join("src", "support", "array")));
  ...
  expect(folderName(pathWithSep)).toBe(basePath(path.join("src", "support", "array")));
});
